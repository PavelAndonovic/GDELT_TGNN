...
if __name__ == '__main__':
    ...
    for year in range(1979, 2015):
        ...
        date_list = [base + datetime.timedelta(days=x) for x in range(356)]
        ...
        temp['Verbal Cooperation'] = temp['QuadClass'].apply(lambda x: 1 if x == 1 else 0)
        temp['Material Cooperation'] = temp['QuadClass'].apply(lambda x: 1 if x == 2 else 0)
        temp['Verbal Conflict'] = temp['QuadClass'].apply(lambda x: 1 if x == 3 else 0)
        temp['Material Conflict'] = temp['QuadClass'].apply(lambda x: 1 if x == 4 else 0)
        ...
        temp = temp.groupby(["Actor1CountryCode", "Actor2CountryCode"]).agg(
            {"Actor1CountryCode": "max", "Actor2CountryCode": "max", "AlternateGoldsteinScale": "sum",
             "SQLDATE": "max", 'Material Cooperation': "mean", 'Verbal Cooperation': "mean", 'Verbal Conflict': "mean",
             'Material Conflict': "mean", "GoldsteinScale": "mean", "NumArticles": "sum",
